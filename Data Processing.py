...
for C_N in range(0, 4):
    ...
    if isExists:
        ...
        if CsvName[C_N] == 'Arm':
            ...
            data1 = pd.read_csv(Path_Raw + '/' + Personnumber + '/' + CsvName[C_N] + '0.csv', header=None,
                                index_col=False,
                                names=names)
        # Add headers
        else:
...
for E_N in range(0, len(EEGName)):
    ...
    ************************************************************************************************
    ...
    for Fnum in range(0, len(Fstartmin)):
        Fstarttime = str(
            VedioStartTime[task] + datetime.timedelta(minutes=int(Fstartmin[Fnum]), seconds=Fstartsec[Fnum]))
    ...
    for TimeDetect in range(0, len(Label)):
        if
    Fstarttime in TimeArray[TimeDetect]:
    ...
    for Labelloc in range(0, len(Label)):
        if
    ((Labelloc >= StarttimeRow) and (Labelloc <= EndtimeRow)):
    Label[Labelloc] = 1
    ...
    endTime = datetime.datetime.strptime('0:' + str(Fendmin[K]) + ':' + str(Fendsec[K]),
                                         '%H:%M:%S') + datetime.timedelta(seconds=1)
    ...
    for K in range(0, len(StartHour)):
        startTime = datetime.datetime.strptime(str(StartHour[K]) + ':' + str(StartMin[K]) + ':' + str(StartSec[K]),
                                               '%H:%M:%S')
